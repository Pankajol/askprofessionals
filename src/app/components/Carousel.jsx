...
const Carousel = () => {
  ...
  const getItemsToShow = () => {
    ...
    return 1; // Small screens (1 image)
  };

  const [itemsToShow, setItemsToShow] = useState(getItemsToShow());

  // Update the number of images to show when the screen size changes
  const handleResize = () => {
    ...
    <div className="relative w-full">
      <div className="flex overflow-hidden justify-center">
        ...
        {carouselItems.slice(activeIndex, activeIndex + itemsToShow).map((item, idx) => (
          <div key={idx} className={`flex-shrink-0 px-2 ${itemsToShow === 1 ? 'w-full' : itemsToShow === 2 ? 'w-1/2' : 'w-1/3'}`}>
